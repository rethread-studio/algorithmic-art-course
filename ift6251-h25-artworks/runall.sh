...
while true; do
    ...
    random_subdir="${subdirs[$RANDOM % ${#subdirs[@]}]}"  # Random selection
    ...
    if [[ -f "$start_script" ]]; then
        ...
        # Change directory to the selected subdirectory and run start.sh with a timeout of d seconds
        (cd "$random_subdir" && timeout $d bash ./start.sh)
