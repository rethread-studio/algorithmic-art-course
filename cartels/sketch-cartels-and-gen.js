...
function drawCartel(x, y, width, height) {
    ...
    //if the data file has less data points than the number of slots on the page, we add gen art :)
    else {
        genart(x, y, width, height) 
    }
    currentCartel++;
}

function genart(x, y, width, height){
    var cx,cy,diam,rad,x1,x2,y1,y2,angle,maxangleinc,xoff,yoff
    cx=x+width*0.5
    cy=y+height*0.5
    diam=width*0.8
    rad=diam*0.5
    ...
    yoff=40
    while(angle<180){
        x1=cx+rad*cos(radians(90+angle))
        x2=cx+rad*cos(radians(90-angle))
        y1=cy+rad*sin(radians(90+angle))
        y2=cy+rad*sin(radians(90-angle))
        //line(x1,y1,x2,y2)
        beginShape();
        vertex(x1,y1);
        bezierVertex(x1+rad*xoff, y1-yoff, x2-rad*xoff, y2+yoff, x2,y2);
        endShape();
        xoff+=random(0.02)
        yoff+=random(2)
        angle+=random(maxangleinc)
    }
