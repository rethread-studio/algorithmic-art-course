...
- ```exquisite.json```: the configuration file that includes all generative artworks. Each artwork is defined as follows: the name of the code file for the artwork ("art_code"), the name of function that initializes the artwork ("art_set") and the name of the function that actually generates the artwork ("art_gen")
...
- the art works: this repo has only 2 works that can serve as examples. These works can be any generative artwork. Only two constraints: (i) don't forget to translate at the location of the O_currentsection; (ii) don't redefine functions names ```draw``` or ```setup```